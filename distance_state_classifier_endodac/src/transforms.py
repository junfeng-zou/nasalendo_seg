...
def normalization_stats(normalize_cfg: dict | None = None) -> tuple[np.ndarray, np.ndarray]:
    cfg = normalize_cfg or {}
    mode = str(cfg.get("mode", "imagenet")).lower()
    if mode in {"imagenet", "official"}:
        return IMAGENET_MEAN, IMAGENET_STD
    if mode == "none":
        return np.zeros(3, dtype=np.float32), np.ones(3, dtype=np.float32)
    if mode == "custom":
        return _as_float_array(cfg.get("mean", IMAGENET_MEAN), "mean"), _as_float_array(cfg.get("std", IMAGENET_STD), "std")
    raise ValueError(f"Unsupported normalization mode: {mode}")


def resize_and_normalize(image_rgb: np.ndarray, size: int, normalize_cfg: dict | None = None) -> np.ndarray:
    resized = cv2.resize(image_rgb, (int(size), int(size)), interpolation=cv2.INTER_AREA)
    arr = resized.astype(np.float32) / 255.0
    mean, std = normalization_stats(normalize_cfg)
    arr = (arr - mean) / std
