...
from distance_state_classifier_endodac.src.dataset import DistanceStateDataset, class_counts
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the geometry-aware endoscope distance-state classifier.")
    ...
    parser.add_argument(
        "--init-checkpoint",
        default=None,
        help="Optional checkpoint used only to initialize model weights. Optimizer/scheduler are not loaded.",
    )
...
def make_class_weights(dataset: DistanceStateDataset, device: torch.device, config: dict) -> torch.Tensor:
    ...
    if configured is not None:
        if isinstance(configured, dict):
            ...
            if values.shape[0] != len(dataset.classes):
                raise ValueError("train.class_weights length must match the number of classes")
    ...
    torch.save(
        {
            ...
            "classes": classes,
            "input_size": int(get_nested(config, "image.input_size", 392)),
            "model_name": str(model_cfg.get("name", "endodac_encoder_classifier")),
            ...
            "pool_type": getattr(model, "pool_type", model_cfg.get("pool_type", "mean")),
            "normalize_mode": str(get_nested(config, "image.normalize.mode", "imagenet")),
            "trainable_scope": model_cfg.get("trainable_scope", "head_only"),
            "config": config,
        },
        path,
    )
...
def apply_epoch_trainable_scope(model: nn.Module, config: dict, epoch: int) -> dict[str, int]:
    model_cfg = dict(get_nested(config, "model", {}))
    final_scope = str(model_cfg.get("trainable_scope", "head_only"))
    freeze_epochs = int(get_nested(config, "train.freeze_encoder_epochs", 0))
...
def main() -> None:
    ...
    output_dir = ensure_dir(get_nested(config, "project.output_dir", "distance_state_classifier_endodac/runs/default"))
    ...
    model_cfg = dict(get_nested(config, "model", {}))
    model = build_model(
        name=str(model_cfg.get("name", "endodac_encoder_classifier")),
    ...
    if args.init_checkpoint:
        ...
        compatible_state = {
            key: value
            for key, value in init_checkpoint["model_state"].items()
            if key in current_state and current_state[key].shape == value.shape
        }
        skipped = len(init_checkpoint["model_state"]) - len(compatible_state)
        ...
        print(f"[init] loaded model weights from {args.init_checkpoint}")

    final_scope = str(model_cfg.get("trainable_scope", "head_only"))
    set_trainable_scope(model, final_scope, last_blocks=int(model_cfg.get("last_blocks", 2)))
    groups = make_optimizer_param_groups(
        model,
    ...
    if not groups:
        raise RuntimeError("No trainable parameters selected. Check model.trainable_scope.")
    optimizer = torch.optim.AdamW(groups, weight_decay=float(get_nested(config, "train.weight_decay", 1e-4)))
    ...
    print("=" * 72)
    print("Geometry-aware distance-state classifier training")
    print(f"output_dir: {output_dir}")
    print(f"device: {device}")
    print(f"model: {model_cfg.get('name', 'endodac_encoder_classifier')}")
    print(f"backbone: {model_cfg.get('backbone', '')}")
    print(f"encoder_backend: {get_nested(config, 'model.encoder.backend', '')}")
    print(f"input_size: {input_size}")
    print(f"normalize: {get_nested(config, 'image.normalize.mode', 'imagenet')}")
    ...
    print(f"trainable_scope: {final_scope}")
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        frozen_now = epoch <= int(get_nested(config, "train.freeze_encoder_epochs", 0))
        ...
        train_metrics = classification_metrics(train_matrix, classes)
