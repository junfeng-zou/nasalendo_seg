...
from distance_state_classifier_endodac.src.predictor import DistanceStatePredictor, Prediction
from distance_state_classifier_endodac.src.transforms import apply_crop
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run EndoDAC distance-state inference on a camera stream.")
    ...
    parser.add_argument("--yolo-model", default=None, help="YOLO segmentation checkpoint for mask-aware inference.")
    ...
    parser.add_argument(
        "--no-instrument-gate",
        action="store_true",
        help="Disable YOLO no-instrument gating for non-mask-aware inference. Mask-aware models still require YOLO masks.",
    )
...
def make_writer(path: str | None, fps: float, frame_size: tuple[int, int]) -> cv2.VideoWriter | None:
    if not path:
        return None
...
def default_record_path() -> Path:
    ...
    return resolve_path(f"distance_state_classifier_endodac/runs/camera_record_{stamp}.mp4")
...
class YoloMaskGenerator:
    def __init__(self, args: argparse.Namespace, config: dict) -> None:
        try:
            ...
            raise SystemExit("Mask-aware inference requires ultralytics in the active environment.") from exc
...
def format_probabilities(result: Prediction | None) -> str:
    ...
    return "  ".join(f"{label}:{prob:.2f}" for label, prob in result.probabilities.items())
...
def main() -> None:
    ...
    mask_generator = YoloMaskGenerator(args, predictor.config) if predictor.mask_aware or instrument_gate_enabled else None
    ...
    if predictor.mask_aware and mask_generator is not None:
        print(f"[camera] mask-aware inference enabled, yolo={mask_generator.model_path}")
    elif instrument_gate_enabled and mask_generator is not None:
    ...
    try:
        while True:
            ...
            measured_fps = (len(recent_times) - 1) / max(recent_times[-1] - recent_times[0], 1e-6) if len(recent_times) >= 2 else 0.0

            if (frame_index - 1) % infer_every == 0:
                t0 = time.time()
                ...
                if no_instrument:
                    ...
                    last_result = predictor.predict_array(frame, apply_raw_crop=args.apply_crop, mask=last_mask)
            ...
            base_view = apply_crop(frame, predictor.crop) if args.display_crop else frame
            mask_view = None
            ...
            preview = cv2.resize(view, None, fx=args.preview_scale, fy=args.preview_scale, interpolation=cv2.INTER_AREA) if args.preview_scale > 0 and args.preview_scale != 1.0 else view

            if not args.no_display:
                cv2.imshow("EndoDAC Distance State Camera Inference", preview)
            ...
            if recording:
                if writer is None:
                    writer = make_writer(record_path.as_posix(), source_fps, (view.shape[1], view.shape[0]))
            ...
            if args.print_every > 0 and frame_index % args.print_every == 0:
                ...
                print(f"[camera] frame={frame_index} state={state} capture_fps={args.print_every / elapsed:.2f} infer_ms={last_latency_ms or 0.0:.1f}")
    ...
    if writer is not None:
        print(f"[camera] wrote annotated recording: {record_path}")
